...
(function (global) {
  ...
  function comicPage8() {
    ...
    /* 페이지에 그려진 빈 말풍선 — 읽는 순서
     * [x, y, w, h, 몇 번째 컷인지, 기본 대사]
     * 기본 대사가 빈 칸은 쓰는 사람이 채우는 자리다. 5·6컷의 반응 대사는
     * 이 밈에서 늘 같은 문구라 미리 채워 둔다. */
    var bubbles = [
      [9, 0, 96, 86, 1, ''],
      [347, 0, 112, 70, 2, ''],
      [7, 214, 93, 130, 3, ''],
    ...
    // 한 컷에 말풍선이 여럿이면 이름에 순번을 붙여 사이드바에서 찾기 쉽게 한다
    var perPanel = {};
    bubbles.forEach(function (bb) { perPanel[bb[4]] = (perPanel[bb[4]] || 0) + 1; });
    var seen = {};

    bubbles.forEach(function (bb) {
      var panel = bb[4];
      seen[panel] = (seen[panel] || 0) + 1;
      var name = panel + '컷 대사' + (perPanel[panel] > 1 ? ' ' + seen[panel] : '');
  ...
  /* ── 7) 삼형제 6컷 ──────────────────────────────────
   * 이 페이지에는 말풍선이 그려져 있지 않다. 그래서 테두리 없는 흰 박스를
   * 직접 얹는다(bubble:'box'). 칸 경계는 그림에 있으므로 panels 는 비운다.
   */
  function brothers6() {
    var W = 594, H = 674;
    ...
    boxes.forEach(function (bx, i) {
      slots.push(textSlot({
        ...
        text: bx[4],
        bubble: 'box',   // 그림에 말풍선이 없으니 테두리 없는 흰 박스를 얹는다
        tail: 'none',
        fontSize: 20,
        autoFit: true
      }));
